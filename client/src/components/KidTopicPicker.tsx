...
export type KidTopicItem<T extends string | number> = { id: T; label: string; detail: string; Icon: LucideIcon };
...
export default function KidTopicPicker<T extends string | number>({ value, onChange, items, disabled = {} }: Props<T>) {
  return <div className="mq-kid-topic" role="radiogroup" aria-label="選擇題型">{items.map(({ id, label, detail, Icon }) => <button key={id} onClick={() => !disabled[id] && onChange(id)} disabled={disabled[id]} aria-pressed={value === id} role="radio"><span className="mq-kid-topic-icon">{disabled[id] ? <Lock /> : <Icon />}</span><strong>{label}</strong><small>{disabled[id] ? "先過關" : detail}</small></button>)}</div>;
}
