...
type MultiplicationQuestion = { groups: number; each: number; choices: number[]; note: string; color: string };
type ResultState = "idle" | "correct" | "incorrect";

const questions: MultiplicationQuestion[] = [
  { groups: 3, each: 4, choices: [7, 12, 14, 16], note: "3 組，每組有 4 個。", color: "#0e8b87" },
  { groups: 5, each: 2, choices: [8, 10, 12, 15], note: "5 組，每組有 2 個。", color: "#c8811e" },
  { groups: 4, each: 3, choices: [9, 10, 12, 14], note: "4 組，每組有 3 個。", color: "#4f6eae" },
  { groups: 2, each: 6, choices: [8, 10, 12, 14], note: "2 組，每組有 6 個。", color: "#b15979" },
  { groups: 3, each: 5, choices: [12, 14, 15, 18], note: "3 組，每組有 5 個。", color: "#f05a3c" },
];
...
export default function P2Practice() {
  ...
  const answer = question.groups * question.each;
  ...
  const restart = () => { setCurrentIndex(0); setResult("idle"); setSelected(null); setScore(0); setStreak(0); setSeconds(0); setFinished(false); };
    ...
    <div className="mq-practice min-h-screen bg-[#f8f5ed] text-[#172b3f] dark:bg-[#101b27] dark:text-[#f4f7f4]">
      <header className="mq-practice-header sticky top-0 z-50 border-b border-[#172b3f]/10 bg-[#f8f5ed]/92 backdrop-blur-xl dark:border-white/10 dark:bg-[#111c28]/92">
        <div className="mx-auto flex h-[72px] max-w-[1280px] items-center justify-between px-5 lg:px-8">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            <span className="hidden font-mono text-[11px] font-bold tracking-[0.1em] text-[#617286] dark:text-[#b7c8ce] sm:block">九九乘法表</span>
      ...
      <main className="mx-auto max-w-[1280px] px-5 py-8 lg:px-8 lg:py-10">
        ...
        <div className="mb-7 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div><div className="flex items-center gap-2 font-mono text-[11px] font-bold tracking-[0.16em] text-[#c8811e]"><span className="size-2 rounded-full bg-[#c8811e]" /> P2 · 乘法站 01</div><h1 className="mt-3 text-3xl font-black tracking-[-0.055em] sm:text-4xl">九九乘法表</h1><p className="mt-2 text-sm leading-6 text-[#617286] dark:text-[#b7c8ce]">看清每一組有多少，再把相同的數量乘起來。</p></div>
          <div className="mq-progress-label rounded-2xl border border-[#172b3f]/10 bg-white px-4 py-3 text-sm dark:border-white/10 dark:bg-[#172737]"><span className="font-mono text-[10px] font-bold tracking-[0.12em] text-[#f05a3c]">今日學習路徑</span><p className="mt-1 font-extrabold">已完成 {completed} / {questions.length} 題 <span className="mx-1 text-[#f05a3c]">·</span> {timeLabel}</p></div>
        </div>
        ...
        <div className="grid gap-5 lg:grid-cols-[230px_minmax(0,1fr)]">
          <aside className="mq-practice-sidebar order-2 lg:order-1">
            <div className="rounded-[24px] border border-[#172b3f]/10 bg-[#172b3f] p-5 text-white shadow-[0_10px_0_#0e1d2a] dark:border-white/10 dark:bg-[#1b3042]">
              <p className="font-mono text-[10px] font-bold tracking-[0.16em] text-[#f6be5d]">本次任務</p><h2 className="mt-3 text-xl font-black leading-7">完成 5 個乘法站。</h2><p className="mt-3 text-sm leading-6 text-white/75">先數清楚每一組，再選出正確的乘法答案。</p>
              <div className="mq-mission-steps mt-5">{questions.map((_, index) => <span key={index} className={index < completed ? "mq-mission-step-done" : index === currentIndex ? "mq-mission-step-current" : ""}>{index + 1}</span>)}</div>
          ...
          <section className="mq-practice-card mq-p2-card order-1 min-h-[540px] overflow-hidden rounded-[28px] border border-[#172b3f]/10 bg-white p-5 shadow-[0_16px_35px_rgba(23,43,63,0.07)] dark:border-white/10 dark:bg-[#172737] md:p-8 lg:order-2">
            {!finished ? <>
              <div className="mq-question-head relative flex items-center justify-between border-b border-[#172b3f]/10 pb-5 dark:border-white/10"><div><p className="font-mono text-[10px] font-bold tracking-[0.14em] text-[#c8811e]">第 {currentIndex + 1} 題 · 乘法站 {String(currentIndex + 1).padStart(2, "0")}</p><p className="mt-1 text-sm font-bold text-[#617286] dark:text-[#b7c8ce]">先找出有幾組，再數每組有多少。</p></div><span className="grid size-10 place-items-center rounded-full bg-[#fff7e9] font-mono text-xs font-black text-[#c8811e] dark:bg-[#3a3325]">{currentIndex + 1}</span></div>
              <div className="flex min-h-[245px] flex-col items-center justify-center py-7 text-center"><div className="mq-question-note"><span className="font-mono">分組提示</span><span>{question.note}</span></div><GroupBoard {...question} /><div className="mt-6 flex items-center justify-center gap-3 font-mono text-[clamp(3rem,7vw,5rem)] font-medium leading-none tracking-[-0.08em]"><span>{question.groups}</span><span className="text-[#f05a3c]">×</span><span>{question.each}</span><span className="text-[#617286] dark:text-[#b7c8ce]">=</span><span className="mq-answer-input grid min-w-[88px] place-items-center rounded-2xl border-2 border-[#172b3f]/15 text-[#172b3f] dark:border-white/20 dark:text-white">{selected ?? "?"}</span></div></div>
