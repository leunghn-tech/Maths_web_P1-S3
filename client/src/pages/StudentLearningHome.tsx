...
export default function StudentLearningHome() {
  ...
  const startHref = reviews[0]?.href ?? getStudentStartHref(savedGrade);
  ...
  return <main className="min-h-screen bg-[#f8f5ed] px-5 py-6 text-[#172b3f] sm:px-8 sm:py-9">
    <div className="mx-auto max-w-6xl">
      ...
      <section className="mt-7 overflow-hidden rounded-[32px] bg-[#172b3f] p-6 text-white shadow-[10px_10px_0_rgba(23,43,63,.1)] sm:p-9">
        ...
        <div className="mt-3 flex flex-col gap-5 md:flex-row md:items-end md:justify-between"><div><h1 className="text-4xl font-black tracking-[-.055em]">你好，{displayName}！</h1><p className="mt-3 max-w-xl leading-7 text-white/75">先看你的進度，再選一個課題開始。錯得較多的課題已放在下面，方便你一步一步重溫。</p></div><Link href={startHref} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-2xl bg-[#f05a3c] px-6 py-4 font-black text-white shadow-[0_4px_0_#c84932] transition hover:-translate-y-0.5"><Play className="size-4 fill-current" />{reviews.length ? "先重溫這一題型" : "開始答題"}</Link></div>
      </section>
      ...
      <section className="mt-6 grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
        ...
        <aside className="rounded-[28px] border border-[#172b3f]/12 bg-[#fffdf8] p-5 sm:p-7"><p className="font-mono text-[10px] font-bold tracking-[.15em] text-[#0e8b87]">QUICK START</p><h2 className="mt-2 text-2xl font-black">現在開始</h2><p className="mt-2 text-sm leading-6 text-[#617286]">你上次選擇的年級會成為開始位置；也可隨時返回年級題庫。</p><Link href={startHref} className="mt-5 flex items-center justify-between rounded-2xl bg-[#0e8b87] px-5 py-4 font-black text-white"><span>{reviews.length ? reviews[0].title : "開始今天的練習"}</span><Play className="size-4 fill-current" /></Link><Link href="/account" className="mt-3 flex items-center justify-between rounded-2xl border border-[#172b3f]/15 bg-white px-5 py-4 font-bold"><span>查看完整進度與同步</span><ChevronRight className="size-4" /></Link></aside>
      </section>
    </div>
  </main>;
}
