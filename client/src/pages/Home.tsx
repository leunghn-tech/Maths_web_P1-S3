...
export default function Home() {
  ...
  const isGradeCompleted = (grade: string) => {
    ...
    if (grade === "P2") return ["p2-numbers", "p2-number-line", "p2-money", "p2-time", "p2-multiply", "p2-divide", "p2-mixed"].some((id) => completedPractices.includes(id));
            ...
            <div className="mq-course rounded-[28px] border border-[#172b3f]/10 bg-white p-5 shadow-[0_12px_30px_rgba(23,43,63,0.06)] md:p-7">
              <div className="flex flex-col gap-5 border-b border-[#172b3f]/10 pb-6 sm:flex-row sm:items-center sm:justify-between">
                ...
                {course.grade === "P1" ? (
                  ...
                  <div className="flex flex-wrap justify-end gap-2"><Link href="/practice/p2-numbers" className="mq-start inline-flex shrink-0 items-center justify-center gap-2 rounded-full bg-[#c8811e] px-5 py-3 text-sm font-extrabold text-white shadow-[0_4px_0_#996019] transition hover:-translate-y-0.5 active:translate-y-0 active:shadow-none"><Play className="size-4 fill-current" /> 三位數</Link><Link href="/practice/p2-numbers-compare" className="mq-library-return inline-flex items-center gap-2 rounded-full border border-[#172b3f]/15 px-4 py-3 text-sm font-extrabold dark:border-white/15">數線比較</Link><Link href="/practice/p2-money" className="mq-library-return inline-flex items-center gap-2 rounded-full border border-[#172b3f]/15 px-4 py-3 text-sm font-extrabold dark:border-white/15">港幣找錢</Link><Link href="/practice/p2-time" className="mq-library-return inline-flex items-center gap-2 rounded-full border border-[#172b3f]/15 px-4 py-3 text-sm font-extrabold dark:border-white/15">時分秒</Link></div>
                ) : course.grade === "P3" ? (
