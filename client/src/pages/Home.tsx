...
export default function Home() {
  ...
  const isGradeCompleted = (grade: string) => {
    ...
    if (grade === "P4") return ["p4-fractions", "p4-decimals", "p4-convert"].some((id) => completedPractices.includes(id));
    return false;
  };
  const courseCompleted = isGradeCompleted(course.grade);
  const courseCompletionLabel = course.grade === "P2" ? `${["p2-multiply", "p2-divide", "p2-mixed"].filter((id) => completedPractices.includes(id)).length}/3 題型完成` : course.grade === "P3" ? `已完成 ${["p3-level-1", "p3-level-2", "p3-level-3"].filter((id) => completedPractices.includes(id)).length}/3 關` : course.grade === "P4" ? `${["p4-fractions", "p4-decimals", "p4-convert"].filter((id) => completedPractices.includes(id)).length}/3 題型完成` : "練習已完成";
            ...
            <div className="mq-course rounded-[28px] border border-[#172b3f]/10 bg-white p-5 shadow-[0_12px_30px_rgba(23,43,63,0.06)] md:p-7">
              <div className="flex flex-col gap-5 border-b border-[#172b3f]/10 pb-6 sm:flex-row sm:items-center sm:justify-between">
                ...
                {course.grade === "P1" ? (
                  ...
                  <Link href="/practice/p4-fractions-decimals" className="mq-start inline-flex shrink-0 items-center justify-center gap-2 rounded-full bg-[#f05a3c] px-5 py-3 text-sm font-extrabold text-white shadow-[0_4px_0_#c84932] transition hover:-translate-y-0.5 active:translate-y-0 active:shadow-none"><Play className="size-4 fill-current" /> 開始 P4 練習</Link>
                ) : (
