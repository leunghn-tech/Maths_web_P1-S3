...
export default function Home() {
  ...
  const isGradeCompleted = (grade: string) => {
    if (grade === "P1") return ["p1-add-subtract", "p1-numbers", "p1-time", "p1-number-line", "p1-measure", "p1-length-compare", "p1-shapes", "p1-solids", "p1-shape-rotation", "p1-length-sort"].some((id) => completedPractices.includes(id));
            ...
            <div className="mq-course rounded-[28px] border border-[#172b3f]/10 bg-white p-5 shadow-[0_12px_30px_rgba(23,43,63,0.06)] md:p-7">
              ...
              {course.grade === "P1" && <section className="mq-p1-recommended mt-6 rounded-2xl border border-[#f05a3c]/20 bg-[#fff7ef] p-4" aria-label="P1 推薦學習次序">
                ...
                <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-5">{[
                  ...
                  { key: "p1-measure", code: "05", icon: "📏", title: "長度與距離", detail: "拖尺子學量度", href: "/practice/p1-measure" },
                ].map((station, stationIndex) => { const done = completedPractices.includes(station.key); const next = !done && ["p1-add-subtract", "p1-numbers", "p1-time", "p1-number-line", "p1-measure"].slice(0, stationIndex).every((key) => completedPractices.includes(key)); return <Link key={station.key} href={station.href} className={`mq-p1-route-card ${next ? "is-next" : ""} ${done ? "is-done" : ""}`}><span className="mq-p1-route-code">{done ? <Check className="size-3" /> : station.code}</span><span className="text-lg leading-none">{station.icon}</span><strong>{station.title}</strong><small>{done ? "已收集星星" : next ? "下一站，按這裡" : station.detail}</small></Link>; })}</div>
              </section>}
