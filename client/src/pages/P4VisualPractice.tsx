...
const fractionQuestions = [
  { left: 1, right: 2, denominator: 4, answer: "3/4", shaded: 3, choices: ["3/4", "1/4", "2/4", "4/4"] }, { left: 1, right: 1, denominator: 3, answer: "2/3", shaded: 2, choices: ["2/3", "1/3", "3/3", "1"] },
  { left: 2, right: 1, denominator: 5, answer: "3/5", shaded: 3, choices: ["3/5", "1/5", "2/5", "4/5"] }, { left: 1, right: 1, denominator: 2, answer: "1", shaded: 2, choices: ["1", "0", "1/2", "3/2"] },
  { left: 2, right: 1, denominator: 4, answer: "3/4", shaded: 3, choices: ["3/4", "1/4", "2/4", "4/4"] }, { left: 1, right: 2, denominator: 6, answer: "1/2", shaded: 3, choices: ["1/2", "1/6", "2/6", "4/6"] },
  { left: 2, right: 1, denominator: 3, answer: "1", shaded: 3, choices: ["1", "0", "1/3", "4/3"] }, { left: 1, right: 3, denominator: 5, answer: "4/5", shaded: 4, choices: ["4/5", "1/5", "3/5", "1"] },
];
...
export default function P4VisualPractice() {
  ...
  const config = mode === "grid" ? { key: "p4-grid-area", title: "方格面積建構", detail: "長方形面積 = 長 × 闊", href: "/practice/p4-grid-area" } : { key: "p4-fractions-visual", title: "同分母分數加法", detail: "分母相同，只把分子相加", href: "/practice/p4-fractions-visual" };
  ...
  const prompt = mode === "grid" ? `用方格建構長 ${grid.width} 格、闊 ${grid.height} 格的長方形。面積是多少平方格？` : `${fraction.left}/${fraction.denominator} 加 ${fraction.right}/${fraction.denominator} 等於多少？`;
  ...
  const next = () => {
    ...
    setIndex((value) => value + 1); setSelectedCells(0); setResult("idle");
  };
