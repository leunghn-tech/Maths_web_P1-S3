...
export const reduce = (numerator: number, denominator: number) => { const divisor = gcd(Math.abs(numerator), denominator); const top = numerator / divisor; const bottom = denominator / divisor; return bottom === 1 ? chineseNumber(top) : `${chineseNumber(bottom)}分之${chineseNumber(top)}`; };
...
export function generateUnlikeFractions(difficulty: Difficulty): RandomProblem {
  ...
  return { id: `unlike-${firstNumerator}-${firstDenominator}-${symbol}-${secondNumerator}-${secondDenominator}-${Math.random()}`, prompt: "先通分，再計算，並把答案約成最簡分數。", equation: `${reduce(firstNumerator, firstDenominator)} ${symbol} ${reduce(secondNumerator, secondDenominator)}`, answer, choices: choicesFor(numerator, common), hint: `先把兩個分數化成分母為 ${common} 的等值分數，再處理分子。` };
}
