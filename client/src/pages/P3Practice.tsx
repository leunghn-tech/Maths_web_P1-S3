...
import { getCompletedPractices, markPracticeCompleted } from "@/lib/practiceCompletion";
...
const getUnlockedLevel = (): Level => {
  ...
  return completed.includes("p3-level-2") ? 3 : completed.includes("p3-level-1") ? 2 : 1;
};
const initialLevel = (): Level => {
  const requested = typeof window === "undefined" ? 1 : Number(new URLSearchParams(window.location.search).get("level"));
  const unlocked = getUnlockedLevel();
  return (requested === 1 || requested === 2 || requested === 3) && requested <= unlocked ? requested : 1;
};
...
function choicesFor(answer: number) { const values = new Set([answer]); for (let distance = 1; values.size < 4; distance += 1) { if (answer - distance >= 1) values.add(answer - distance); if (values.size < 4) values.add(answer + distance); } return Array.from(values).sort(() => Math.random() - 0.5); }
export function generateLevelQuestions(level: Level): Question[] {
  ...
  return Array.from({ length: 8 }, (_, index) => {
    ...
    if (level === 2) { const product = a * b; const extra = 1 + Math.floor(Math.random() * Math.min(cap - 1, product - 1)); const answer = product - extra; return { expression: `${a} × ${b} − ${extra}`, answer, choices: choicesFor(answer), note: `先算 ${a} × ${b}，再減 ${extra}。` }; }
...
export default function P3Practice() {
  ...
  const [unlockedLevel, setUnlockedLevel] = useState<Level>(getUnlockedLevel);
