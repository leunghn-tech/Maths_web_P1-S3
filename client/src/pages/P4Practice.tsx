...
export default function P4Practice() {
  ...
  const voiceReviewItems = wrongIndices.map((itemIndex) => { const item = questions[itemIndex]; return { prompt: `${item.expression}等於幾多`, answer: item.answer, hint: item.note }; });
  ...
  return <div className={`mq-practice mq-p4-practice ${reviewMode ? "mq-review-mode" : ""} min-h-screen bg-[#f8f5ed] text-[#172b3f] dark:bg-[#101b27] dark:text-[#f4f7f4]`}><MidpointBreak open={midpointOpen} onContinue={() => { setMidpointOpen(false); setCurrentIndex((value) => value + 1); setSelected(null); setResult("idle"); }} />{reviewMode && !finished && <div className="mq-review-hint-overlay"><VisualReviewHint kind={mode === "fractions" ? "fraction" : "application"} expression={question.expression} answer={question.answer} hint={question.note} accent={question.color} /></div>}{finished && !reviewMode && <FinishAchievementOverlay perfect={activeIndices.length === 8 && score === 8} items={voiceReviewItems} />}
