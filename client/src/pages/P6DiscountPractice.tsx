...
function generateDiscount(difficulty: Difficulty): RandomProblem {
  ...
  if (difficulty === "standard") {
    return { id: `discount-${price}-${discount}-${Math.random()}`, prompt: `一件物品原價 ${money(price)}，現正 ${discount}% off，折後價是多少？`, equation: `${price} − ${discountAmount}`, answer: money(salePrice), choices: createChoices(salePrice, [discountAmount, price + discountAmount, salePrice + 10]), hint: "先找出折扣金額，再由原價減去折扣。" };
  }
