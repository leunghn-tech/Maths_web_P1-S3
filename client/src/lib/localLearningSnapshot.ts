...
export function hasLocalLearningData(snapshot = getLocalLearningSnapshot()) {
  return snapshot.completedPractices.length > 0 || Object.keys(snapshot.dailyHistory).length > 0 || snapshot.reviewRecords.length > 0 || snapshot.pinnedPractices.length > 0;
}
